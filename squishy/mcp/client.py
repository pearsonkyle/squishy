...
class StdioTransport:
    ...
    def start(self) -> None:
        base_env = {k: v for k, v in os.environ.items() if k in self._SAFE_ENV_KEYS}
        env = {**base_env, **(self._config.env or {})}
...
class MCPManager:
    ...
    def add_server(self, config: MCPServerConfig) -> MCPClient:
        ...
        sanitized = "".join(c if c.isalnum() or c == "_" else "_" for c in config.name)
